...
# File Information: This function handles tail, which
# displays the last few lines of a file. By default,
  ...
  try:
    ...
    if numberSpecified is not None:
      if numberSpecified <= len(param[0]):
        for index in range(len(param[0]) - numberSpecified,\
        len(param[0])):
      ...
      if height <= len(param[0]):
        for index in range(len(param[0]) - height , \
        len(param[0])):
          displayList.append(param[0][index])
      else:
        for index in range(0, len(param[0])):
...
if __name__ == '__main__':
  ...
  print(testSort)
  print(tail(parameters = ["test.txt"], \
  flags = ["--help", "-n"]))
  print(tail(parameters = ["test.txt", "20"], \
  flags = ["-n"]))
  print(tail(parameters = ["test.txt"], \
  flags = []))

  print(tail(parameters = ["commands/stresstest.txt"], \
  flags = []))

  print(tail(parameters = ["commands/stresstest.txt", 20], \
  flags = ["-n"]))